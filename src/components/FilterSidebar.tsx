...
export default function FilterSidebar() {
  ...
  const filterCategories = [
    ...
    },
    {
      id: 'type',
      name: 'Jewellery Type',
      options: ['Necklaces', 'Earrings', 'Bangles & Bracelets', 'Rings', 'Mangalsutra']
    },
    {
      id: 'color',
      name: 'Accent Color',
      options: ['Ruby Red', 'Emerald Green', 'Sapphire Blue', 'Polki/Kundan', 'Antique Gold']
    },
    {
      id: 'price',
      name: 'Price Range',
    ...
    <>
      ...
      `}>
        <div className="flex items-center justify-between p-4 border-b border-gray-100 md:hidden pb-[17px] sticky top-0 bg-white z-10">
          ...
          <button onClick={() => setIsOpen(false)} className="p-2 -mr-2 text-gray-500 hover:text-brand-maroon">
            <X className="h-6 w-6" />
          </button>
        </div>
        ...
        <div className="p-5">
          <div className="hidden md:flex items-center justify-between mb-6">
            ...
            <button className="text-sm text-brand-maroon hover:underline">Clear All</button>
          </div>
            ...
            {filterCategories.map((category) => (
              <div key={category.id} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                <button className="flex items-center justify-between w-full mb-3 group">
                  <h3 className="font-medium text-sm tracking-wide uppercase text-gray-900 group-hover:text-brand-maroon transition-colors">{category.name}</h3>
